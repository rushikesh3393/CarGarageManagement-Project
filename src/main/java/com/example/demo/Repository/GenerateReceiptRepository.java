...
public class GenerateReceiptRepository {
    ...
    {
        int value = template.update("insert into invoice values (?, ?, ?, ?, ?, ?, ?, ?, ?)", new PreparedStatementSetter() 
        {
    ...
    {
        List<GenerateReceiptModel> rList=template.query("select *from invoice",new RowMapper<GenerateReceiptModel> () 
        {
    ...
    {
    	List<GenerateReceiptModel> rList=template.query("select *from invoice where apid=?",new Object[] {apid},new RowMapper<GenerateReceiptModel> () 
        {
			public GenerateReceiptModel mapRow(ResultSet rs, int rowNum) throws SQLException 
			{
				GenerateReceiptModel gmodel=new GenerateReceiptModel();
    ...
    {
    	int value=template.update("delete from invoice where apid="+apid);
    	if(value>0)
    	{
    		return true;
    	}
    ...
    {
    	int value=template.update("update invoice set servicetype=?,sparename=?,spareqty=? where apid=?",new PreparedStatementSetter () 
    	{
    		public void setValues(PreparedStatement ps) throws SQLException 
			{
    ...
    {
    	List<GenerateReceiptModel> recList=template.query("select *from invoice where cname like '%"+recPattern+"%' ", new RowMapper<GenerateReceiptModel> () {
